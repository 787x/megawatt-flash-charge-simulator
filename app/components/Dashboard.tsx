...
export function Dashboard() {
  ...
  useEffect(() => {
    if (!config.vehicleModels.some((m) => m.id === selectedVehicleModelId)) {
      setSelectedVehicleModelId(config.vehicleModels.find((m) => m.id === DEFAULT_FLASH_MODEL_ID)?.id ?? config.vehicleModels[0]?.id ?? "");
    }
  ...
  const selectScenario = (name: string) => {
    ...
    setConfig(next);
    setSelectedVehicleModelId(DEFAULT_FLASH_MODEL_ID);
  ...
  const importScenario = async (file?: File) => {
    ...
    try {
      ...
      setSelectedVehicleModelId(result.config.vehicleModels.find((m) => m.id === DEFAULT_FLASH_MODEL_ID)?.id ?? result.config.vehicleModels[0]?.id ?? "");
      reset(result.config);
      setToast("场景导入成功");
    } catch (error) {
