    ...
    <div className={`app-shell ${mobileUiMode === "task" ? "mobile-ui-task" : "mobile-ui-legacy"} ${activeDialog ? "overlay-open" : ""}`}>
      ...
      <header className="mobile-status-header">
        ...
        <div className="mobile-header-actions">
          {isElectronDesktop && <label className="zoom-control zoom-control-compact" aria-label="显示缩放"><select value={zoomPercent} onChange={(event) => setZoomPercent(Number(event.target.value))}>{ZOOM_LEVELS.map((level) => <option value={level} key={level}>{level}%</option>)}</select></label>}
      ...
      {showHelp && <div className="modal-backdrop" onMouseDown={() => setShowHelp(false)}><section className="modal help-modal" data-dialog-id="help" role="dialog" aria-modal="true" aria-labelledby="help-title" onMouseDown={(event) => event.stopPropagation()}>
        <div className="help-modal-header"><h2 id="help-title">兆瓦闪充站仿真 · 使用说明</h2><button className="modal-close" data-initial-focus aria-label="关闭帮助" onClick={() => setShowHelp(false)}>×</button></div>
        <div className="help-modal-body">
          ...
          <div className="help-sections">
            ...
            <section><h3>这个工具在模拟什么</h3><p>本项目的灵感来源于比亚迪 2026 年 3 月发布的新一代闪充技术及"闪充站中站"方案。相比单纯提高充电桩峰值功率，这类方案更有意思的地方在于：利用储能系统，在原有电网接入能力有限的情况下，为车辆提供远高于电网实时供电能力的瞬时充电功率，从而使部分既有充电站具备升级为超高功率闪充站的可能。</p><p>这也带来了一个很自然的问题：一个原本只有有限配电容量的充电站，加入储能以后，真的能够长期支撑兆瓦级充电吗？</p><p>单次闪充并不难理解。电网提供一部分功率，储能在短时间内补足剩余部分即可。真正复杂的是连续运营。</p><p>车辆不会按照固定时间到达，每辆车的车型、初始 SOC、目标 SOC 和充电曲线也不同；车辆高 SOC 后充电功率会下降，两辆车之间还存在换车间隙；多个充电位可能同时工作，部分车位又可能具有车型或品牌使用限制，例如闪充桩右侧车位是闪充专用，而左侧是通用，因此在排队、时间预估上都需要考虑这一点。与此同时，储能既要在高负荷时放电，又需要利用低负荷和车辆充电功率下降的时间重新补能。</p><p>因此，这实际上是一个由电网、储能、充电设施、车辆、排队和调度策略等共同构成的动态系统。</p></section>
        ...
        <div className="help-modal-footer">
          <div className="help-copyright"><p>Copyright (C) 2026 Wang Shengyu</p><p>Licensed under the GNU General Public License v3.0 only.</p><p>Source code: <a href="https://github.com/787x/megawatt-flash-charge-simulator" target="_blank" rel="noopener noreferrer">https://github.com/787x/megawatt-flash-charge-simulator</a></p></div>
